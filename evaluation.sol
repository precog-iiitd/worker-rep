...
contract evaluation is workerTaskPosterContract {
  ...
  function randomNumberGen(uint _modulus) internal returns(uint) {
      randNonce++;
      //uint rand = uint(keccak256((now+randNonce), msg.sender, randNonce)) % _modulus;
      //randArray.push(rand);
    ...
    uint[][3] public workerInRepRange;
  ...
  function _findingEvaluator(uint _numberOfEvalutor, uint _agreementId) {
      ...
      //int kk = _numberOfEvalutor;
      //uint[] memory workerRange = new uint[](_numberOfEvalutor);
